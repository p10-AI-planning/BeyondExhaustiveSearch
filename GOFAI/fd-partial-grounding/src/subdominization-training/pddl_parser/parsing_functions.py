...
def parse_typed_list(alist, only_variables=False,
                     constructor=gofai_pddl.TypedObject,
                     default_type="object"):
    result = []
    while alist:
        try:
...
def parse_predicate(alist):
    ...
    return gofai_pddl.Predicate(name, arguments)


def parse_function(alist, type_name):
    name = alist[0]
    arguments = parse_typed_list(alist[1:])
    return gofai_pddl.Function(name, arguments, type_name)
...
def parse_condition_aux(alist, negated, type_dict, predicate_dict):
    ...
    if tag == "and" and not negated or tag == "or" and negated:
        return gofai_pddl.Conjunction(parts)
    elif tag == "or" and not negated or tag == "and" and negated:
        return gofai_pddl.Disjunction(parts)
    elif tag == "forall" and not negated or tag == "exists" and negated:
        return gofai_pddl.UniversalCondition(parameters, parts)
    elif tag == "exists" and not negated or tag == "forall" and negated:
        return gofai_pddl.ExistentialCondition(parameters, parts)
...
def parse_literal(alist, type_dict, predicate_dict, negated=False):
    ...
    if negated:
        return gofai_pddl.NegatedAtom(pred_id, alist[1:])
    else:
        return gofai_pddl.Atom(pred_id, alist[1:])
...
def add_effect(tmp_effect, result):
    ...
    if isinstance(tmp_effect, gofai_pddl.ConjunctiveEffect):
        for effect in tmp_effect.effects:
            add_effect(effect, result)
        return
    else:
        parameters = []
        condition = gofai_pddl.Truth()
        if isinstance(tmp_effect, gofai_pddl.UniversalEffect):
            parameters = tmp_effect.parameters
            if isinstance(tmp_effect.effect, gofai_pddl.ConditionalEffect):
                condition = tmp_effect.effect.condition
                assert isinstance(tmp_effect.effect.effect, gofai_pddl.SimpleEffect)
                effect = tmp_effect.effect.effect.effect
            else:
                assert isinstance(tmp_effect.effect, gofai_pddl.SimpleEffect)
                effect = tmp_effect.effect.effect
        elif isinstance(tmp_effect, gofai_pddl.ConditionalEffect):
            condition = tmp_effect.condition
            assert isinstance(tmp_effect.effect, gofai_pddl.SimpleEffect)
            effect = tmp_effect.effect.effect
        else:
            assert isinstance(tmp_effect, gofai_pddl.SimpleEffect)
            effect = tmp_effect.effect
        assert isinstance(effect, gofai_pddl.Literal)
        # Check for contradictory effects
        condition = condition.simplified()
        new_effect = gofai_pddl.Effect(parameters, condition, effect)
        contradiction = gofai_pddl.Effect(parameters, condition, effect.negate())
        if not contradiction in result:
            result.append(new_effect)
        else:
            # We use add-after-delete semantics, keep positive effect
            if isinstance(contradiction.literal, gofai_pddl.NegatedAtom):
...
def parse_effect(alist, type_dict, predicate_dict):
    tag = alist[0]
    if tag == "and":
        return gofai_pddl.ConjunctiveEffect(
            [parse_effect(eff, type_dict, predicate_dict) for eff in alist[1:]])
    elif tag == "forall":
        assert len(alist) == 3
        parameters = parse_typed_list(alist[1])
        effect = parse_effect(alist[2], type_dict, predicate_dict)
        return gofai_pddl.UniversalEffect(parameters, effect)
    elif tag == "when":
        assert len(alist) == 3
        condition = parse_condition(
            alist[1], type_dict, predicate_dict)
        effect = parse_effect(alist[2], type_dict, predicate_dict)
        return gofai_pddl.ConditionalEffect(condition, effect)
    elif tag == "increase":
        assert len(alist) == 3
        assert alist[1] == ['total-cost']
        assignment = parse_assignment(alist)
        return gofai_pddl.CostEffect(assignment)
    else:
        # We pass in {} instead of type_dict here because types must
        # be static predicates, so cannot be the target of an effect.
        return gofai_pddl.SimpleEffect(parse_literal(alist, {}, predicate_dict))


def parse_expression(exp):
    if isinstance(exp, list):
        functionsymbol = exp[0]
        return gofai_pddl.PrimitiveNumericExpression(functionsymbol, exp[1:])
    elif exp.replace(".", "").isdigit():
        return gofai_pddl.NumericConstant(float(exp))
    elif exp[0] == "-":
        raise ValueError("Negative numbers are not supported")
    else:
        return gofai_pddl.PrimitiveNumericExpression(exp, [])
...
def parse_assignment(alist):
    ...
    if op == "=":
        return gofai_pddl.Assign(head, exp)
    elif op == "increase":
        return gofai_pddl.Increase(head, exp)
    else:
...
def parse_action(alist, type_dict, predicate_dict):
    ...
    if precondition_tag_opt == ":precondition":
        ...
        if not precondition_list:
            # Note that :precondition () is allowed in gofai_pddl.
            precondition = gofai_pddl.Conjunction([])
        else:
    ...
    for rest in iterator:
        assert False, rest
    if eff:
        return gofai_pddl.Action(name, parameters, len(parameters),
...
def parse_axiom(alist, type_dict, predicate_dict):
    ...
    return gofai_pddl.Axiom(predicate.name, predicate.arguments,
...
def parse_task(domain_pddl, task_pddl):
    ...
    requirements = gofai_pddl.Requirements(sorted(set(
    ...
    init += [gofai_pddl.Atom("=", (obj.name, obj.name)) for obj in objects]

    return gofai_pddl.Task(
        domain_name, task_name, requirements, types, objects, constants,
...
def parse_domain_pddl(domain_pddl):
    ...
    requirements = gofai_pddl.Requirements([":strips"])
    the_types = [gofai_pddl.Type("object")]
    ...
    for opt in iterator:
        ...
        if field == ":requirements":
            requirements = gofai_pddl.Requirements(opt[1:])
        elif field == ":types":
            the_types.extend(parse_typed_list(
                    opt[1:], constructor=gofai_pddl.Type))
        elif field == ":constants":
            constants = parse_typed_list(opt[1:])
        elif field == ":predicates":
            the_predicates = [parse_predicate(entry)
                              for entry in opt[1:]]
            the_predicates += [gofai_pddl.Predicate("=",
                                 [gofai_pddl.TypedObject("?x", "object"),
                                  gofai_pddl.TypedObject("?y", "object")])]
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    yield gofai_pddl.Requirements(requirements)
