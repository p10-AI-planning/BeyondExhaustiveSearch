        ...
        for rule in self.rules:
            ...
            if not eff_vars.issubset(cond_vars):
                ...
                for var in sorted(eff_vars):
                    rule.add_condition(gofai_pddl.Atom("@object", [var]))
        if must_add_predicate:
            print("Unbound effect variables: Adding @object predicate.")
            self.facts += [Fact(gofai_pddl.Atom("@object", [obj])) for obj in self.objects]
    ...
    def convert_trivial_rules(self):
        ...
        for i, rule in enumerate(self.rules):
            if not rule.conditions:
                ...
                self.add_fact(gofai_pddl.Atom(rule.effect.predicate, rule.effect.args), rule.weight)
        ...
        for i, var_name in enumerate(atom.args):
            if var_name[0] == "?":
                if var_name in used_variables:
                    ...
                    new_conditions.append(gofai_pddl.Atom("=", [var_name, new_var_name]))
                else:
...
def translate_typed_object(prog, obj, type_dict):
    ...
    for type_name in [obj.type_name] + supertypes:
        prog.add_fact(gofai_pddl.TypedObject(obj.name, type_name).get_atom())
...
def translate_facts(prog, task):
    ...
    for fact in task.init:
        assert isinstance(fact, gofai_pddl.Atom) or isinstance(fact, gofai_pddl.Assign)
        if isinstance(fact, gofai_pddl.Atom):
            prog.add_fact(fact)
...
def add_inequalities(prog,task):
    for obj1 in task.objects:
        for obj2 in task.objects:
            if obj1 == obj2:
                continue
            a = gofai_pddl.Atom(normalize.NOT_EQUAL_PREDICATE, [obj1.name, obj2.name])
            prog.add_fact(a, 0)
