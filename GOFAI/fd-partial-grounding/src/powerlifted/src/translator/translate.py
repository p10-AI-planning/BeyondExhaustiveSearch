...
def main():
    ...
    assert isinstance(task.goal, gofai_pddl.Conjunction) or \
           isinstance(task.goal, gofai_pddl.Atom) or \
           isinstance(task.goal, gofai_pddl.NegatedAtom), \
...
def print_action_schemas(task, object_index, predicate_index, type_index):
    ...
    for action in task.actions:
        ...
        if action.cost is None:
            action.cost = 0
        if isinstance(action.cost, gofai_pddl.Increase):
            if isinstance(action.cost.expression, gofai_pddl.NumericConstant):
        ...
        for cond in sorted(precond):
            assert isinstance(cond, gofai_pddl.Literal)
            args_list = []
            for x in cond.args:
        ...
        for eff in action.effects:
            assert isinstance(eff, gofai_pddl.Effect)
            args_list = []
...
def print_goal(task, atom_index, object_index, predicate_index):
    ...
    if isinstance(task.goal, gofai_pddl.Conjunction):
...
def print_initial_state(task, atom_index, object_index, predicate_index):
    ...
    new_s0 = []
    for a in task.init:
        if isinstance(a, gofai_pddl.conditions.Atom):
            new_s0.append(a)
    ...
    for index, atom in enumerate(task.init):
        ...
        if isinstance(atom, gofai_pddl.Assign):
            continue
        print(atom, '%d %d %d %d' % (
...
def remove_static_predicates_from_goal(task, static_pred):
    parts = []
    removed = 0
    if isinstance(task.goal, gofai_pddl.conditions.Atom) or \
       isinstance(task.goal, gofai_pddl.Atom) or \
       isinstance(task.goal, gofai_pddl.NegatedAtom):
    ...
    if len(parts) == 0:
        ...
        sys.exit(0)
    task.goal = gofai_pddl.Conjunction(parts)
...
def is_trivially_unsolvable(task, static_pred):
    ...
    if isinstance(task.goal, gofai_pddl.conditions.Atom) or isinstance(task.goal, gofai_pddl.Atom):
...
def remove_functions_from_initial_state(task):
    new_init = []
    for i in task.init:
        if not isinstance(i, gofai_pddl.Assign):
            new_init.append(i)
    for i in new_init:
        assert (isinstance(i, gofai_pddl.Atom))
    task.init = new_init
