...
def compile_into_unary_predicates(task):
    ...
    for t in task.types:
        ...
        task.predicates.append(gofai_pddl.Predicate(pred_name, ['?x']))
    ...
    for action in task.actions:
        ...
        for obj in obj_in_action:
            ...
            action.precondition.add_condition(
                gofai_pddl.Atom(_get_type_predicate_name(param_type), [name]))
    return
...
def adjust_initial_state(task, graph):
    ...
    for obj in task.objects:
        ...
        for t in types:
            task.init.append(gofai_pddl.Atom(_get_type_predicate_name(t), [name]))
