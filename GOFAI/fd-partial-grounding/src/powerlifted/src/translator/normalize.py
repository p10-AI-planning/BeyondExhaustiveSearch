...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules,add_inequalities):
        ...
        eff_rule_head = gofai_pddl.Atom(axiom.name, [par.name for par in params])
...
class GoalConditionProxy(ConditionProxy):
    ...
    def delete_owner(self, task):
        ...
        assert False, "Disjunctive goals not (yet) implemented."
    def build_rules(self, rules, add_inequalities):
        rule_head = gofai_pddl.Atom("@goal-reachable", [])
        rule_body = condition_to_rule_body([], self.condition, add_inequalities)
        rules.append((rule_body, rule_head, None))
    def get_type_map(self):
        # HACK!
...
def get_action_predicate(action):
    ...
    if isinstance(action.precondition, gofai_pddl.ExistentialCondition):
        variables += [par.name for par in action.precondition.parameters]
    return gofai_pddl.Atom(name, variables)

def get_axiom_predicate(axiom):
    name = axiom
    variables = [par.name for par in axiom.parameters]
    if isinstance(axiom.condition, gofai_pddl.ExistentialCondition):
...
def remove_duplicated_preconditions(task):
    for action in task.actions:
        precond = set()
        if not isinstance(action.precondition, gofai_pddl.Conjunction):
            continue
...
def remove_universal_quantifiers(task):
    def recurse(condition):
        ...
        if isinstance(condition, gofai_pddl.UniversalCondition):
            axiom_condition = condition.negate()
            parameters = sorted(axiom_condition.free_variables())
            typed_parameters = tuple(gofai_pddl.TypedObject(v, type_map[v]) for v in parameters)
            ...
            return gofai_pddl.NegatedAtom(axiom.name, parameters)
        else:
...
def build_DNF(task):
    def recurse(condition):
        ...
        for part in condition.parts:
            part = recurse(part)
            if isinstance(part, gofai_pddl.Disjunction):
        ...
        if isinstance(condition, gofai_pddl.Disjunction):
            result_parts = other_parts
            for part in disjunctive_parts:
                result_parts.extend(part.parts)
            return gofai_pddl.Disjunction(result_parts)

        # Rule (2): Distributivity disjunction/existential quantification.
        if isinstance(condition, gofai_pddl.ExistentialCondition):
            parameters = condition.parameters
            result_parts = [gofai_pddl.ExistentialCondition(parameters, (part,))
                            for part in disjunctive_parts[0].parts]
            return gofai_pddl.Disjunction(result_parts)

        # Rule (3): Distributivity disjunction/conjunction.
        assert isinstance(condition, gofai_pddl.Conjunction)
        result_parts = [gofai_pddl.Conjunction(other_parts)]
        while disjunctive_parts:
            ...
            for part1 in previous_result_parts:
                for part2 in parts_to_distribute:
                    result_parts.append(gofai_pddl.Conjunction((part1, part2)))
        return gofai_pddl.Disjunction(result_parts)
...
def split_disjunctions(task):
    for proxy in tuple(all_conditions(task)):
        ...
        if isinstance(proxy.condition, gofai_pddl.Disjunction):
...
def move_existential_quantifiers(task):
    def recurse(condition):
        ...
        for part in condition.parts:
            part = recurse(part)
            if isinstance(part, gofai_pddl.ExistentialCondition):
        ...
        if isinstance(condition, gofai_pddl.ExistentialCondition):
            ...
            return gofai_pddl.ExistentialCondition(new_parameters, new_parts)
        ...
        new_conjunction = gofai_pddl.Conjunction(new_conjunction_parts)
        return gofai_pddl.ExistentialCondition(new_parameters, (new_conjunction,))
...
def eliminate_existential_quantifiers_from_axioms(task):
    ...
    for axiom in task.axioms:
        ...
        if isinstance(precond, gofai_pddl.ExistentialCondition):
...
def substitute_complicated_goal(task):
    goal = task.goal
    if isinstance(goal, gofai_pddl.Literal):
        return
    elif isinstance(goal, gofai_pddl.Conjunction):
        for item in goal.parts:
            if not isinstance(item, gofai_pddl.Literal):
                break
        else:
            return
    new_axiom = task.add_axiom([], goal)
    task.goal = gofai_pddl.Atom(new_axiom.name, new_axiom.parameters)
...
def condition_to_rule_body(parameters, condition, add_inequalities):
    ...
    if not isinstance(condition, gofai_pddl.Truth):
        if isinstance(condition, gofai_pddl.ExistentialCondition):
            for par in condition.parameters:
                result.append(par.get_atom())
            condition = condition.parts[0]
        if isinstance(condition, gofai_pddl.Conjunction):
            parts = condition.parts
        else:
            parts = (condition,)
        for part in parts:
            if isinstance(part, gofai_pddl.Falsity):
                # Use an atom in the body that is always false because
                # it is not initially true and doesn't occur in the
                # head of any rule.
                return [gofai_pddl.Atom("@always-false", [])]
            assert isinstance(part, gofai_pddl.Literal), "Condition not normalized: %r" % part
            if not part.negated:
                result.append(part)
            else:
