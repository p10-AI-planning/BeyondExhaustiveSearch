...
def evaluate_inigoal_rule(rule, fact_list):
    ...
    for fact in fact_list:
        if type(fact) != gofai_pddl.Assign and fact.predicate == predicate_name and eval_constants(fact, constants):
            values = []
            for a in arguments:
    ...
    def __init__(self, rule_text, task):
        ...
        for rule in body.split(";"):
            ...
            if rule_type == "ini":
                ...
                arguments, compliant_values = evaluate_inigoal_rule(rule, task.split_relaxed_plan_facts)
            elif rule_type == "relaxed_fact":
                arguments, compliant_values = evaluate_inigoal_rule(rule, task.split_relaxed_plan_facts)
