...
class Action:
    ...
    def _compute_add_del_atom_ids_if_necessary(self, instance_info):
        if not self.add_atom_ids and not self.del_atom_ids:
            ...
            for eff in self.action.predicate.effects:
                ...
                assert eff.condition == gofai_pddl.conditions.Truth()
    ...
    def get_precondition_atom_ids(self, instance_info):
        if not self.precondition_atom_ids:
            self._compute_params_map_if_necessary()
            if isinstance(self.action.predicate.precondition, gofai_pddl.conditions.Atom):
                ...
                assert isinstance(self.action.predicate.precondition, gofai_pddl.conditions.Conjunction)
...
class PolicyQueue(PriorityQueue):
    def __init__(self, task, policy_file):
        ...
        assert(isinstance(task.goal, gofai_pddl.conditions.Conjunction) or
               isinstance(task.goal, gofai_pddl.conditions.Atom))
        ...
        # add goal predicates
        if isinstance(task.goal, gofai_pddl.conditions.Atom):
