...
def translate_strips_conditions_aux(conditions, dictionary, ranges):
    ...
    for fact in conditions:
        if fact.negated:
            ...
            new_condition = {}
            atom = gofai_pddl.Atom(fact.predicate, fact.args)  # force positive
...
def pddl_to_sas(task):
    ...
    for item in goal_list:
        assert isinstance(item, gofai_pddl.Literal)
...
def get_relaxed_plan_facts(relaxed_plan_file):        
    fact_list = []
    with open(relaxed_plan_file, "r") as f:
        for line in f.readlines():
            if not line.startswith(";"):
                tokens = line.strip().strip("(").strip(")").split(" ")
                fact_list.append(gofai_pddl.Atom(tokens[0],tokens[1:]))
    return fact_list
...
def main():
    ...
    with timers.timing("Parsing", True):
        ...
        if (options.relaxed_plan_file):
            task.split_relaxed_plan_facts = get_relaxed_plan_facts(options.relaxed_plan_file)
