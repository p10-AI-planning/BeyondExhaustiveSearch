...
import subdominization.queue_factory as qf
...
def variables_to_numbers(effect, conditions):
    ...
    for i, arg in enumerate(effect.args):
        if arg[0] == "?":
            rename_map[arg] = i
            new_effect_args[i] = i
    new_effect = gofai_pddl.Atom(effect.predicate, new_effect_args)
    ...
    for cond in conditions:
        ...
        new_conditions.append(gofai_pddl.Atom(cond.predicate, new_cond_args))
...
class Queue:
    def __init__(self, atoms, task):
        ...
        if (options.hard_rules):
            self.action_queue = hr.get_hard_rules_from_options(qf.get_action_queue_from_options(task))
        else:
    ...
    def push(self, predicate, args):
        ...
        if eff_tuple not in self.enqueued:
            ...
            atom = gofai_pddl.Atom(predicate, list(args))
            if isinstance(predicate, gofai_pddl.Action):
