...
def instantiate_goal(goal, init_facts, fluent_facts):
    ...
    # gofai_pddl.condition or a similar file, not here. The `instantiate`
    ...
    except gofai_pddl.conditions.Impossible:
        return None
    return result
...
def instantiate(task, model):
    ...
    for element in task.init:
        if isinstance(element, gofai_pddl.Assign):
    ...
    for atom in model:
        if isinstance(atom.predicate, gofai_pddl.Action):
        ...
        elif isinstance(atom.predicate, gofai_pddl.Axiom):
