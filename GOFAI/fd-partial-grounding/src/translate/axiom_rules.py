...
class AxiomDependencies(object):
    def __init__(self, axioms):
        if DEBUG:
            assert all(isinstance(axiom.effect, gofai_pddl.Atom) for axiom in axioms)
        ...
        if cluster.needed_negatively:
            if len(cluster.variables) > 1:
                ...
                for variable in cluster.variables:
                    ...
                    negated_axiom = gofai_pddl.PropositionalAxiom(name, [], variable.negate())
...
def negate(axioms):
    assert axioms
    result = [gofai_pddl.PropositionalAxiom(axioms[0].name, [], axioms[0].effect.negate())]
    for axiom in axioms:
